...
namespace TheOuterWoldShopHome.Pages
{
    public partial class Listing : System.Web.UI.Page
    {
        ...
        public IEnumerable<Home> GetGames()
        {
            return FilterGames()
                .OrderBy(g => g.HomeId)
        ...
        private IEnumerable<Home> FilterGames()
        {
            IEnumerable<Home> homes = repository.Homes;
            string currentCategory = (string)RouteData.Values["category"] ??
                Request.QueryString["category"];
            return currentCategory == null ? homes :
                homes.Where(p => p.Category == currentCategory);
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (IsPostBack)
            {
                ...
                {
                    Home selectedGame = repository.Homes
                        .Where(g => g.HomeId == selectedGameId).FirstOrDefault();
